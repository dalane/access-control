...
import { makeCompilePolicy, IPolicy } from '../app/policy/index';
...
import { ICompileSpecificationFunc, ISpecificationMatchFunc } from '../app/policy/specification';
...
describe('compiling a policy', () => {
  ...
  const _compileSpecification = (result?:boolean):ICompileSpecificationFunc=> (value:any):ISpecificationMatchFunc => (accessRequest:IAccessRequest):boolean => result;
  ...
  it('throws an error if policy version is not equal to 1', () => {
    ...
    const mockCompileResource = _compileResource();
    const mockCompileSpecification = _compileSpecification();
    const itThrows = () => makeCompilePolicy(mockCompileAction)(mockCompileResource)(mockCompilePrincipal)(mockCompileSpecification)(merge({}, DefaultAllowPolicy, { version: 2 }));
  ...
  it('it returns a compiled policy object that returns expected results', () => {
    ...
    const sut = makeCompilePolicy(mockCompileAction)(mockCompileResource)(mockCompilePrincipal)(mockCompileSpecification)(DefaultAllowPolicy);
